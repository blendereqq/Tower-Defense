...
#define VERSION "1.1 Beta Admin Fix"
#define AUTHOR "GT Team & blendeR"
...
public plugin_init() {
        ...
        register_clcmd("gold_ammount_admin", "GiveGoldAdmin")
}
 
public Menu(id)
{
        /* Jeśli jest to admin, to daj mu prawo otwarcia menu */
        ...
        if(td_get_user_info(id, PLAYER_GOLD) <= 0) {
                ColorChat(id, GREEN, "%s^x01 Masz za malo Golda", CHAT_PREFIX)
                return
        }
        if(get_playersnum() == 0) {
        ...
        toMenu:
        /* Jeśli ma prawo do otwarcia, pokaz liste graczy*/
...
{
        /* Jeśli wybrał gracza, to wybierz ile masz dać */
        ...
        formatex(szTitle, charsmax(szTitle), "Wybrany gracz: \r%s^n\wIle wyslac?",  szTarget)
...
public Menu2H(id, menu, item)
{
        ...
        if(item == 0) { 
                ...
                client_print(id, print_center, "Wpisz kwote ktora chcesz przekazac");
...
public menuAdmin(id, id2) {     
        ...
        if(!is_user_alive(id2)) {
                ColorChat(id, GREEN, "%s^x01 Gracz '%s' nie zyje!", CHAT_PREFIX, szTargetNick)
                Menu(id)
                return
        }
                
        formatex(szTitle, charsmax(szTitle), "Wybrany gracz: \r%s^n\wIle wyslac?",  szTargetNick)
...
public GiveGoldAdmin(id)
{
        if((get_user_flags(id) & ADMIN_CVAR))
        {
                ...
                ColorChat(id, GREEN, "%s^x01 Wartosc '%d' jest zbyt niska wpisz poprawnie!", CHAT_PREFIX, str_to_num(szAmmount))
...
public GiveGold(id)
{
        ...
        if(str_to_num(szAmmount) > td_get_user_info(id, PLAYER_GOLD)) {
                ColorChat(id, GREEN, "%s^x01 Nie posiadasz tyle'%d' GOLDa", CHAT_PREFIX, str_to_num(szAmmount))
                Menu(id) 
                return;
...
ChangeGold(this, target, ammount, admin = 0)
{
        ...
        if(!is_user_connected(target)) {
                ColorChat(this, GREEN, "%s^x01 Gracz nieaktywny '%s' Brak polaczenia!", CHAT_PREFIX,  szName)
                return;
        }
        ...
        if(!admin)
                ColorChat(this, GREEN, "^x04%s^x01 Wyslales ^x04%d ^x01gold do ^x04'%s'!", CHAT_PREFIX,  ammount, szName)
        else
                ColorChat(0, GREEN, "^x04%s^x01 Admin ^x04'%s' ^x01wyslal ^x04%d ^x01gold do ^x04'%s'!", CHAT_PREFIX,  szAdminName,  ammount, szName)
	       log_to_file("addons/amxmodx/logs/td-gold.log", "ADMIN %s przekazał %d dla %s",szAdminName, ammount,szName);
        if(admin)
                ColorChat(target,  GREEN, "^x04%s^x01 Otrzymales od admina ^x04'%s' %d ^x01gold", CHAT_PREFIX, szAdminName, ammount)
        else
                ColorChat(0,  GREEN, "^x04%s^x01 Gracz ^x04'%s' ^x01wyslal ^x04'%s' %d ^x01gold!", CHAT_PREFIX, szAdminName, szName, ammount)
	       log_to_file("addons/amxmodx/logs/td-gold.log", "%s przekazał %d dla %s",szAdminName, ammount,szName);
